...
it('说明重载失败保留此前完整有效目录',()=>{const app=setup();const path=join(app.store.directory,'device-capabilities.json');writeFileSync(path,readFileSync('../../protocol/examples/capabilities/demo-device.json'));const loaded=app.reloadCapabilities();expect(loaded.active?.devices).toHaveLength(1);writeFileSync(path,'bad JSON');const failed=app.reloadCapabilities();expect(failed.error).not.toBeNull();expect(failed.active).toEqual(loaded.active);});
