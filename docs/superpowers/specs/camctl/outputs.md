...
outputs 记录本专题定义的稳定身份、来源、可用性、派生关系和清理状态。deliveries 记录 `delivery_id`、`output_id`、来源 obtain 动作、历史唯一的实际 `file_name`、发布历史、撤回历史及交付状态。文件位置与业务记录之间的对账引用[文件交接](file-handoff.md)。
