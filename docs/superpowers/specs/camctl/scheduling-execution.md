...
| `obtain_action_outputs` | 必填 | 首次受理时按静态参数错误处理 |
| `report_status`、`cancel_task` | 可省略 | 执行会话取得相应执行资格后立即处理，无须等待指定的计划时间 |
| `delete_action_outputs` | 支持指定时间，省略规则待明确 | 见[取回与清理的执行时间](outputs.md#取回与清理的执行时间) |

允许省略时，“立即”仍须遵守动作的执行条件及会话模式；`submit` 只受理，不执行这些动作。时钟异常期间新取消动作的资格仍按 U4 确认，不能仅凭省略时间字段取得执行资格。显式填写时间时，按该时间调度。首次受理的静态参数错误按请求专题的整份计划校验规则处理；已受理 ID 的重试继续复用原计划。
...
按已明确的动作类型覆盖 `scheduled_at` 缺省与显式填写：设备动作及取回缺省时静态校验失败；报告与取消缺省时不等待计划时间，显式填写时遵守调度时间。立即处理仍受执行资格约束，不能让 `submit` 执行动作。
