...
客户端本地状态丢失后的请求入口、ACK 输入及较早完整报告先被确认的例子见[完整状态同步推演](status-sync.md#完整同步时先收到较早的报告)；保留较旧完整基础时，从指定报告之后补齐的完整 JSON 见[局部同步样例](examples/status-sync/README.md)。这些例子沿用本专题的对象合并规则，不重放设备动作。
