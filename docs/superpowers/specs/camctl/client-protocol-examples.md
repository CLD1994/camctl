...
[返回设计总览](../2026-09-08-camctl-cli-design.md) · [请求受理与会话](protocol-session.md) · [状态报告与累计确认](status-reports.md) · [阅读路线](reading-guide.md)
...
**阅读提示：** 第一次阅读先看样例一的操作经过及结果，再打开对应计划与报告 JSON。随后按需阅读参数错误、部分失败、取消与清理；字段参考集中在[报告字段契约](report-format.md)。

本页导航：

- [阅读与使用范围](#阅读与使用范围)
- [文件入口](#文件入口)
- [样例一：录像与取回成功](#样例一录像与取回成功)
- [样例二：单个动作参数错误，其他动作继续执行](#样例二单个动作参数错误其他动作继续执行)
- [样例三：组取回部分失败](#样例三组取回部分失败)
- [样例四：组内一个录像没有产物](#样例四组内一个录像没有产物)
- [样例五：取消完成后清理已有产物](#样例五取消完成后清理已有产物)
- [样例六：取消已生效，但停止没有确认](#样例六取消已生效但停止没有确认)
- [样例字段说明](#样例字段说明)
- [计划文件诊断样例](#计划文件诊断样例)
- [客户端接收与核对](#客户端接收与核对)
...
以上场景均假定状态库、本地文件交接和会话收尾成功；动作失败不表示 camctl 会话失败。第三方主程序只中转文件，不解析下面的业务字段。样例只展示指定的报告机会；实际运行中的其他报告机会及 `ready` 替换继续遵守[报告生成点](status-reports.md#报告生成点)。
...
本场景使用已确认的默认读取上限、无数据超时和独立重拷额度，具体约束见[按文件累计读取尝试](file-copy.md#按文件累计读取尝试)、[文件读取的无数据超时](file-copy.md#文件读取的无数据超时)和[摘要不一致后的有限重拷](file-copy.md#摘要不一致后的有限重拷)。读取重试之间先确认旧读取已经停止，并在等待期间保留该相机的拷贝机会；重试间隔来自已固化的本地配置，遵守[通信重试间隔](configuration.md#通信重试间隔)。
