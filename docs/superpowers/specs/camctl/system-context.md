...
待确认请求、累计 ACK 和重送继续遵守[客户端确认与重送](plan-acceptance.md#客户端确认与重送)。报告导入与 ACK 输入分别经过一次人工交接；在网页中完成导入不表示确认已经送达 camctl。能力说明使用部署时的[手工同步流程](camera-capabilities.md#部署时向客户端提供说明)。
