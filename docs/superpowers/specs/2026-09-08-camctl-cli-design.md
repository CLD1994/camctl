...
使用单个 SQLite 数据库作为权威持久状态。内存缓存可丢弃并从持久化事实重建。请求、动作、效果、设备事实、产物、交付、报告及审计记录的具体字段归各责任专题。

数据库数据永久保留，当前不设计删除或到期淘汰机制。该规则适用于所有已持久化记录及其身份关联，包括业务记录、报告记录和审计记录；正常状态字段仍按各自契约更新，不可变内容继续保持不可变。

计划完成或退役、产物文件清理、交付文件删除及累计 ACK 推进，均不触发数据库记录删除。数据库中的报告内容及 `report_id` 到覆盖水位的关联同样永久保留；存储在数据库之外的计划文件、产物文件和报告文件，继续按各自文件生命周期处理。

数据库与文件系统、设备之间的副作用需要明确可恢复的先后关系。单文件原子移动要求见文件交接专题；各动作的恢复证据见动作专题。
