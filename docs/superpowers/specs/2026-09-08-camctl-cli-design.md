...
第一版以单台主机及其持久化数据库为运行范围。计划、动作、产物、交付和报告的身份及历史均在该范围内解释。

`camctl` 是运行在 ARM 嵌入式 Linux 板上的 Python CLI。第一版接入当前 ADB 相机的录像模式，并为其他拍摄能力、相机模式和 MCU 预留设备与动作接入接口：
...
| [相机拍摄](camctl/camera-recording.md) | 拍摄能力、驱动模式与参数类型；当前 ADB 录像模式的启动、时长、停止、恢复、修复与取消 | 公共执行、正式产物 |
...
相机以拍摄能力、驱动模式和模式参数表达一次拍摄要求。设备可以支持单张照片、录像、延时摄影中的任意一种或多种能力，后续可扩展；每种能力可有一个或多个设备专属模式。客户端将用户预设展开成明确计划后提交，camctl 不管理用户预设。参数及执行契约见[拍摄能力与模式](camctl/camera-recording.md#拍摄能力与模式)。
...
本流程以单台主机、当前 ADB 相机为对象，假设输入合法、主机时间可信、通信正常且存储空间充足。使用[采集与取回计划示例](camctl/protocol-session.md#基本结构)完成一条可直接验收的业务链。
...
| 真实相机接入 | 完成驱动命令映射及主流程联调；驱动替身通过不代表真实设备已经验收 |
...
交接目录配置见文件交接；时钟、设备和调度策略见调度与设备执行；当前录像模式的恢复等待上限见相机拍摄。配置示例值与已确认行为约束分别标明。
