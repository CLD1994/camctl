...
客户端对接可结合[客户端协议样例](camctl/client-protocol-examples.md)阅读：样例提供正常成功、单动作参数错误和组取回部分失败的完整 JSON，以及 ACK 输入和增量合并结果。
...
交接目录配置见文件交接；时钟、设备和调度策略见调度与设备执行；当前相机录像的恢复等待上限见相机拍摄。配置示例值与已确认行为约束分别标明。
