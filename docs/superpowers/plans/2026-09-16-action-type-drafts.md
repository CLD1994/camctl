...
**规格：** [客户端计划编辑](../specs/camctl/client-editing.md)、[数据保存与恢复](../specs/camctl-client/storage-and-recovery.md)。
...
- [x] 在 `tests/unit/web-editing.test.ts` 添加失败用例：首次切换只剩公共字段、往返恢复对象与未完成输入、未知和缺省类型、删除前项后内容不串位、普通字段编辑和追加保留资料、整体 JSON 替换需要显式标志。
- [x] 运行 `npm run test:unit -- tests/unit/web-editing.test.ts`，确认失败来自缺少切换行为。
- [x] 在 `src/server/models.ts`、`src/web/editing.ts` 实现模型和纯转换；`setValue` 保留编辑资料，`removeAction` 同步移动对应资料，整份 JSON 替换显式清除。
- [x] `src/web/session.ts` 的 `sameContent` 比较全部编辑资料；`src/server/application.ts` 检查资料结构，复用现有原子保存。
...
- [x] `src/web/Editor.tsx` 的类型选择器调用转换；整个动作有 pending 时禁用切换；整份 JSON 实际编辑前确认资料替换。
- [x] `src/web/BuiltinFields.tsx` 按识别的精确来源模式显示附加筛选；JSON 原值继续提供诊断与显式移除。
- [x] `src/web/Fields.tsx` 普通字段一直显示，选填已有值/pending 提供清空；保留全部 JSON 值类型。
