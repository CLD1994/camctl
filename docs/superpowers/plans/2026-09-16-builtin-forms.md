...
规格：[客户端计划编辑](../../architecture/client-editing.md#内置动作与公共字段)。协议来源为 plan-input、obtaining-outputs、task-cancellation、output-cleanup 和 status-sync 各专题。
...
- [x] 运行 `npm run test:integration -- --maxWorkers=1 apps/client/tests/integration/browser-editing.test.ts`，确认新测试因未实现表单失败。
