...
const stores: Store[] = [];
function setup() {
  const dir = mkdtempSync(join(tmpdir(), "camctl-db-"));
  dirs.push(dir);
...
describe("客户端数据库启动与事务", () => {
  it("普通启动不创建数据库", () => {
    ...
    expect(store.status().state).toBe("uninitialized");
    expect(existsSync(join(dir, "client.db"))).toBe(false);
  });
  it("显式初始化保存完成事实并可以重开", () => {
    const { dir, store } = setup();
    store.initialize();
    store.set("drafts", "one", { text: '{"name":' });
    store.close();
    const next = new Store(dir);
    stores.push(next);
    expect(next.status().state).toBe("ready");
    expect(next.get("drafts", "one")).toEqual({ text: '{"name":' });
  });
  it("重复初始化不覆盖已保存内容", () => {
    const { store } = setup();
    store.initialize();
    store.set("drafts", "one", { name: "草稿" });
    store.initialize();
    expect(store.get("drafts", "one")).toEqual({ name: "草稿" });
  });
  it("业务文件存在而库缺失时拒绝初始化", () => {
    const { dir, store } = setup();
    writeFileSync(join(dir, "video.mp4"), "video");
    expect(store.status().state).toBe("fault");
    expect(() => store.initialize()).toThrow();
    expect(existsSync(join(dir, "client.db"))).toBe(false);
  });
  it("损坏库保留原文件且拒绝初始化", () => {
    const { dir, store } = setup();
    writeFileSync(join(dir, "client.db"), "invalid");
    expect(store.status().state).toBe("fault");
    expect(() => store.initialize()).toThrow();
  });
  it("能力说明不阻止显式初始化", () => {
    const { dir, store } = setup();
    writeFileSync(join(dir, "device-capabilities.json"), "bad");
    store.initialize();
