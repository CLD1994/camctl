...
export function validatePlan(
  ...
  if (!isObject(plan))
    return [{ path: "", code: "invalid_plan", message: "计划必须是对象" }];
  fields(
    plan,
    ["request_id", "created_at", "name", "actions", "last_report_id"],
    "",
  );
  ...
  actions.forEach((action, index) => {
    ...
    if (!isObject(action)) {
      issue(path, "invalid_action", "动作必须是对象");
      return;
    }
    fields(
      action,
    ...
    if (!ACTION_TYPES.includes(action.type as ActionType)) {
      issue(`${path}.type`, "unsupported_action", "本版不支持此动作类型");
      return;
    }
    ...
    if (Object.hasOwn(action, "group"))
      check(
        type !== "obtain_action_outputs" && isName(action.group),
        `${path}.group`,
        "动作所属组不合法",
      );
    if (isCameraAction(type)) {
      ...
      if (!isObject(action.policy))
        issue(`${path}.policy`, "invalid_policy", "拍摄必须提供策略");
      else {
    ...
    if (type === "report_status") {
      ...
      if (isObject(params.source)) {
        ...
        if (Object.hasOwn(source, "action_name"))
          check(
            actions.some(
              (a) =>
                isObject(a) &&
                a.name === source.action_name &&
                isCameraAction(a.type),
            ),
        ...
        if (
          Object.hasOwn(source, "group") &&
          !Object.hasOwn(source, "plan_instance_id")
        )
          check(
            actions.some(
              (a) =>
                isObject(a) &&
                a.group === source.group &&
                isName(a.group) &&
                isCameraAction(a.type),
            ),
            `${p}.source.group`,
